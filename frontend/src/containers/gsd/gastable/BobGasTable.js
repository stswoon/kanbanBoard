...
export class BobGasTable extends Component {
    state = {
        loading: false,
        points: [],
    ...
    componentDidMount() {
        let points = LocalStorageService.get(BobLocalStorageKeys.geology.gasPoints);
        if (points == null) {
            points = [];
        }
    ...
    calculate = () => {
        ...
        this.setState({warnings, loading: true}, () => {
            ...
            BobOilService.calculate()
                .then((response) => {
                    const points = response.data.gas.points;
    ...
    handleChange = (data) => {
        ...
        LocalStorageService.set(BobLocalStorageKeys.geology.gasPoints, points);
